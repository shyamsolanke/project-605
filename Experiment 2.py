...
# -- Experiment 2: CPU Multi-Thread Setup --
print("\nInitializing Experiment 2: CPU Multi-Thread Setup... \n")
...
print(f"Configuration:\n"
      f"  Device           = {device}\n"
      f"  Intra-threads    = {torch.get_num_threads()}\n"
      f"  Inter-threads    = {torch.get_num_interop_threads()}\n"
      f"  Epochs           = {num_epochs}\n"
    ...
    if enable_profiling and epoch == 1:
        ...
        with profile(
            ...
            for batch_idx, (images, labels) in enumerate(
                tqdm(train_loader, desc="Profiling", total=10)
            ):
        ...
        with open("profiler_epoch1_summary_exp2.txt", "w") as f:
            f.write(summary)
        prof.export_chrome_trace("profiler_epoch1_trace_exp2.json")
        print("Profiler results saved to files.\n")

    # Continue normal training
    for batch_idx, (images, labels) in enumerate(
        tqdm(train_loader, desc=f"Epoch {epoch}/{num_epochs} [Train]", unit="batch")
    ):
...
def eval_model():
    ...
    for batch_idx, (images, labels) in enumerate(
        tqdm(test_loader, desc="Evaluation", unit="batch")
    ):
        images, labels = images.to(device), labels.to(device)
        outputs = model(images)
        loss = criterion(outputs, labels)
    ...
    torch.save(model.state_dict(), 'resnet18_cifar10_cpu_mt.pth')
    print("Model saved as resnet18_cifar10_cpu_mt.pth\n")

if __name__ == '__main__':
    for epoch in range(1, num_epochs+1):
        train_epoch(epoch)
        eval_model()
    print("All done. Experiment 2 complete.")